...
def run_pipeline(pipe, args, prompt: str, image=None, mask_image=None, inpaint: bool=False, cattn_masking: bool=False, 
                ...
                new_generation: bool = True, remove_checkbox: bool = False, init_from_image: bool = False):
    ...
    if remove_checkbox is False:
        result_tensor, latent_memory_new = pipe(
            prompt=prompt,
            image=image if inpaint or init_from_image else None,
            ...
            init_latent_memory_from_image=init_from_image,
        )
    else: 
        result_tensor, latent_memory_new = pipe(
            prompt=prompt,
...
def main():
    ...
    def generate(prompt=None, mask=None, image=None, scheduler=None, dpms_guidance_scale=None, sas_guidance_scale=None, 
        ...
        if args.counter == 0:
            args.counter += 1
            if image is None:
                # Generate background from scratch
                ...
                # Initialize latent memory from provided input image
                print("Using provided image as background")
                image = run_pipeline(pipe_inpaint, args, prompt, image=image, mask_image=None, inpaint=False, 
                                object_gen=False, utilize_cache=False, init_from_image=True)
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                vanilla_ratio_input = gr.Slider(label="Vanilla Ratio", minimum=0.00, maximum=1.0, value=args.vanilla_ratio, step=0.01)
