...
import scripts.mask_utils as mask_utils
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if is_cross and self.attn_masks is not None:
            ...
            # attn_img = (self.attn_masks[0].clamp(0, 1) * 255).to(torch.uint8).cpu().numpy()
            # image = Image.fromarray(attn_img, mode='L')
            # image = image.resize((256, 256), resample=Image.Resampling.BICUBIC)
            # image.save(f"attention_map_first_{cnt}.png")
            # attn_img = (self.attn_masks[1].clamp(0, 1) * 255).to(torch.uint8).cpu().numpy()
            # image = Image.fromarray(attn_img, mode='L')
            # image = image.resize((256, 256), resample=Image.Resampling.BICUBIC)
            # image.save(f"attention_map_second_{cnt}.png")
            # cnt+=1
            for q in self.color_indices[0]:
                # Insted of scale the attention probs of color token with 4 times, I scale it with maximum probs of the color token
                # max_attn_probs_q = attention_probs[:,:,:,q].max().detach()
                # print("max_attn_probs_q",max_attn_probs_q)
                # attention_probs[:,:,:,q][mask_first==1] = (4 * max_attn_probs_q).clamp(0, 1)
                attention_probs[:,:,:,q][mask_first==1]*=4

            for p in self.color_indices[1]:
                # max_attn_probs_p = attention_probs[:,:,:,p].max().detach()
                # print("max_attn_probs_p",max_attn_probs_p)
                # attention_probs[:,:,:,p][mask_second==1] = (4 * max_attn_probs_p).clamp(0, 1)
...
def view_images(images, num_rows=1, offset_ratio=0.02, name='test',conform=False):
    ...
    # display(pil_img)
    import os
    os.makedirs('att_test_0', exist_ok=True)
    pil_img.save('att_test_0/{}_{}.png'.format(name,str(conform)))

def show_cross_attention(tokenizer, prompt, attention_store: AttentionStore, res: int):
    ...
    for i in range(len(tokens)):
        ...
        images.append(image)
    view_images(np.stack(images, axis=0), name=prompt)
...
class PixArtAlphaInpaintLMPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        init_latent_memory_from_image: bool = False,
        ratio=None,
        ...
        **kwargs,
        ...
        # Special mode: Initialize latent memory from input image
        if init_latent_memory_from_image and image is not None:
            print("Initializing latent memory from input image...")
            ...
            # Create latent memory by adding noise at each timestep (forward diffusion)
            latent_memory_new = {}
            noise = randn_tensor(image_latents.shape, generator=generator, device=device, dtype=image_latents.dtype)
            
            for t in timesteps:
                # Add noise to the image latents for this timestep
                noisy_latents = self.scheduler.add_noise(image_latents, noise, t.unsqueeze(0))
                latent_memory_new[int(t.detach().cpu().numpy())] = noisy_latents
            
            # Decode latents back to image for output
            if output_type != "latent":
                output_image = self.vae.decode(image_latents / self.vae.config.scaling_factor, return_dict=False)[0]
                output_image = self.image_processor.postprocess(output_image, output_type=output_type)
            else:
                output_image = image_latents
            
            # Store prompt embeddings for future reference
            if prompt is not None:
                prompt_embeds_tmp, prompt_attention_mask_tmp, _, _ = self.encode_prompt(
                    prompt,
                ...
                # Store in layerstore for background
                self.layerstore._store_layer(
                    torch.cat([prompt_embeds_tmp, prompt_embeds_tmp], dim=0), 
                    torch.cat([prompt_attention_mask_tmp, prompt_attention_mask_tmp], dim=0), 
                    None, None, None, None
                )
            
            elapsed_time = time.time() - start_time
            print(f"Latent memory initialized in {elapsed_time:.2f} seconds")
            
            if not return_dict:
                return (output_image, latent_memory_new)
            return ImagePipelineOutput(images=output_image), latent_memory_new
        ...
        init_image = None
        if inpaint and image is not None:
            # Preprocess the input image for inpainting
            init_image = self.image_processor.preprocess(image, height=height, width=width)
                            ...
                            if multi_query_disentanglement is True: 
                                if self.layerstore._len_of_stored_layers() == 0:
                                    ...
                                    if len(prev_prompt_embeds_list) == 1:
                                        ...
                                        '''
                                        blend foregound masks from step 1.
                                        '''
                                        ...
                                        # to_pil(prev_attention_masks).resize((512,512)).save(f"mask_step{self.layerstore._len_of_stored_layers()}.png")
                ...
                if remove_prev is not True or (remove_prev is True and i >= len(timesteps) - int(num_inference_steps * vanilla_ratio)):
                    ...
                    if remove_prev is not True:
                        if inpaint and i < len(timesteps) - int(num_inference_steps * vanilla_ratio): # minimum (1)
                            ...
                            # Use image_latents if provided (from input image), otherwise use latent_memory (from previous generation)
                            if image_latents is not None:
                                # Add noise to image_latents for proper inpainting at this timestep
                                init_latents_proper = self.scheduler.add_noise(image_latents, noise, torch.tensor([noise_timestep]).to(device))
                                mask_orig_init, _ = mask_orig.chunk(2)
                                # Blend: keep original image in non-masked areas, use denoised latents in masked areas
                                latents = (1 - mask_orig_init) * init_latents_proper + mask_orig_init * latents
                            else:
                                # Fall back to latent_memory for subsequent edits
                                mask_orig_init, _ = mask_orig.chunk(2)
                                latents = (1 - mask_orig_init) * latent_memory[int(t.detach().cpu().numpy())] + mask_orig_init * latents
                    else:
                        if inpaint and i < len(timesteps) - int(num_inference_steps * vanilla_ratio / 2): # minimum (1)
                            # Prepare the latents for the next timestep with inpainting
                            mask_orig_init, _ = mask_orig.chunk(2)
                            
                            # Use image_latents if provided, otherwise use self.latent0
                            # if image_latents is not None:
                            #     noise_timestep = timesteps[i + int(num_inference_steps * vanilla_ratio)] if i + int(num_inference_steps * vanilla_ratio) < len(timesteps) else t
                            #     init_latents_proper = self.scheduler.add_noise(image_latents, noise, torch.tensor([noise_timestep]).to(device))
                            #     latents = (1 - mask_orig_init) * init_latents_proper + mask_orig_init * latents
                            # else:
                            latents = (1 - mask_orig_init) * self.latent0[int(t.detach().cpu().numpy())] + mask_orig_init * latents
        ...
        if self.layerstore._len_of_stored_layers() == 0: # attention_mask for background is WHOLE - mask, so skipping saving for the first time
            ...
            to_pil = torchvision.transforms.ToPILImage()
            to_pil(mask).resize((512,512)).save(f"mask_step{self.layerstore._len_of_stored_layers()}.png")
            to_pil(mask_orig[0]).resize((512,512)).save(f"orig_mask_step{self.layerstore._len_of_stored_layers()}.png")
