...
def main():
    ...
    parser.add_argument('--cattn_masking', action='store_true', default=True, help='Use Cross-Attention masking')
    parser.add_argument('--multi_query_disentanglement', action='store_true', default=True, 
    ...
    # Try loading with local_files_only first, fallback to downloading if needed
    try:
    ...
    for idx, data_ in tqdm(enumerate(val_loader), total=len(val_loader), desc="Generating images"):
        ...
        for layer in range(data_['total_layers']):
            ...
            mask.save(os.path.join(save_dir, f'IDX{idx}_layer{layer}_mask.png'))
            res.resize((128, 128)).save(os.path.join(save_dir, f'IDX{idx}_layer{layer}_lowres.png'))
            
            pipe_inpaint.attention_store.reset()
