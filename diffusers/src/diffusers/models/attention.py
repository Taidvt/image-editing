        ...
        if self.attn2 is not None:
            ...
            if attention_mask is not None: ### ADDED 
                if multi_query_disentanglement is True and prev_attention_masks is not None:
                    ...
                    # Handle dimension properly for attention_mask_org
                    if attention_mask_org.dim() > 1:
                        mask_1d_current = attention_mask_org[0].squeeze()
                    else:
                        mask_1d_current = attention_mask_org.squeeze()
                    
                    max_seq_len = norm_hidden_states.shape[1]
                    non_zero_idx_attn_mask = mask_1d_current.nonzero().reshape(-1)
                    non_zero_idx_attn_mask = non_zero_idx_attn_mask[non_zero_idx_attn_mask < max_seq_len]
                    ...
                    for i in range(prev_attention_masks.shape[0] // attention_mask_org.shape[0]):
                        ...
                        # Handle dimension for prev_attn_mask_iter
                        if prev_attn_mask_iter.dim() > 1:
                            prev_mask_1d = prev_attn_mask_iter[0].squeeze()
                        else:
                            prev_mask_1d = prev_attn_mask_iter.squeeze()
                        non_zero_idx_attn_mask_iter = prev_mask_1d.nonzero().reshape(-1)
                        non_zero_idx_attn_mask_iter = non_zero_idx_attn_mask_iter[non_zero_idx_attn_mask_iter < max_seq_len]
                    ...
                    for i in range(int(prev_attention_masks.shape[0] // attention_mask_org.shape[0]) + 2):
                        if i == 0:
                            ...
                            real_non_zero_idx_attn_mask[i] = torch.arange(0, max_seq_len, device=attention_mask_org.device)
                    ...
                    for i in range(len(real_non_zero_idx_attn_mask)):
                        ...
                        valid_indices = real_non_zero_idx_attn_mask[i][real_non_zero_idx_attn_mask[i] < max_seq_len]
                        masked_query_set[i] = norm_hidden_states[:, valid_indices, :]
                    # process overlapping regions
                    if process_overlap is True:
                        for i in range(1, len(real_non_zero_idx_overlap_attn_mask)+1):
                            masked_overlapped_query_set[i] = []
                            for j in range(len(real_non_zero_idx_overlap_attn_mask[i])):
                                # Ensure indices are within bounds
                                valid_overlap_indices = real_non_zero_idx_overlap_attn_mask[i][j][real_non_zero_idx_overlap_attn_mask[i][j] < max_seq_len]
                                masked_overlapped_query_set[i].append(norm_hidden_states[:, valid_overlap_indices, :])
                    ...
                    masked_output_set = {}
                    for i in range(len(real_non_zero_idx_attn_mask)):
                        # Only capture attention for current object (i==0)
                        capture_kwargs = cross_attention_kwargs.copy() if cross_attention_kwargs else {}
                        capture_kwargs['capture_attention'] = (i == 0)
                    ...
                    if process_overlap is True:
                        ...
                        for i in range(1, len(real_non_zero_idx_overlap_attn_mask)+1):
                            ...
                            for j in range(len(real_non_zero_idx_overlap_attn_mask[i])):
                                ...
                                # Overlapped cross attention - don't capture overlapped regions
                                capture_kwargs = cross_attention_kwargs.copy() if cross_attention_kwargs else {}
                                capture_kwargs['capture_attention'] = False
                    ...
                    for i in range(len(real_non_zero_idx_attn_mask)):
                        ...
                        if len(valid_indices) > 0:
                            attn_output_org[:, valid_indices, :] = masked_output_set[i][:, :len(valid_indices), :]
                    # overlapped part
                    if process_overlap is True:
                        try:
                            keys_view = real_non_zero_idx_overlap_attn_mask.keys()
                            keys_list = list(keys_view)
                            for i in range(len(real_non_zero_idx_overlap_attn_mask)):
                                for j in range(len(real_non_zero_idx_overlap_attn_mask[keys_list[i]])):
                                    # Ensure indices are within bounds
                                    overlap_indices = real_non_zero_idx_overlap_attn_mask[keys_list[i]][j]
                                    valid_overlap_indices = overlap_indices[overlap_indices < max_seq_len]
                                    if len(valid_overlap_indices) > 0:
                                        attn_output_org[:, valid_overlap_indices, :] = overlapped_mask_output_set[keys_list[i]][j][:, :len(valid_overlap_indices), :]
                    ...
                    attn_output_org = norm_hidden_states.clone()
                    # Ensure we're working with the correct dimension
                    # attention_mask_org shape: (batch_size, seq_len) or (batch_size, 1, seq_len)
                    # We need indices for the sequence dimension only
                    if attention_mask_org.dim() > 1:
                        # Take the first batch element and flatten to 1D
                        mask_1d = attention_mask_org[0].squeeze()
                    else:
                        mask_1d = attention_mask_org.squeeze()
                    
                    non_zero_idx_attn_mask = mask_1d.nonzero().reshape(-1) # REVERSED -> Not reversed now # FIXME
                    # Ensure indices are within bounds
                    max_seq_len = norm_hidden_states.shape[1]
                    non_zero_idx_attn_mask = non_zero_idx_attn_mask[non_zero_idx_attn_mask < max_seq_len]
                    query_masked = norm_hidden_states[:, non_zero_idx_attn_mask, :] * alpha
                    
                    zero_idx_attn_mask = mask_1d.eq(0.).nonzero().reshape(-1)
                    zero_idx_attn_mask = zero_idx_attn_mask[zero_idx_attn_mask < max_seq_len]
                    ...
                    #     # thresholding
                    #     mid_rep_mask = ((mid_rep - mid_rep.mean()) / mid_rep_std > sigma).float() # FIXME update sigma on __init__
                    ...
                    # Capture attention only from current object (masked region)
                    capture_kwargs_masked = cross_attention_kwargs.copy() if cross_attention_kwargs else {}
                    capture_kwargs_masked['capture_attention'] = True
                    attn_output_masked = self.attn2(
                        query_masked,
                        encoder_hidden_states=encoder_hidden_states,
                        attention_mask=encoder_attention_mask,
                        **capture_kwargs_masked,
                    )
                    
                    # Don't capture attention from background/previous objects
                    capture_kwargs_unmasked = cross_attention_kwargs.copy() if cross_attention_kwargs else {}
                    capture_kwargs_unmasked['capture_attention'] = False
                    attn_output_unmasked = self.attn2(
                        ...
                        **capture_kwargs_unmasked,
                    )
                    # Ensure indices are within bounds before writing
                    if len(non_zero_idx_attn_mask) > 0:
                        attn_output_org[:, non_zero_idx_attn_mask, :] = attn_output_masked
                    if len(zero_idx_attn_mask) > 0:
                        attn_output_org[:, zero_idx_attn_mask, :] = attn_output_unmasked
                    attn_output = attn_output_org
                
            else: # attention mask is None (regular generation, e.g., background)
                # Capture attention for regular/background generation
                capture_kwargs = cross_attention_kwargs.copy() if cross_attention_kwargs else {}
                capture_kwargs['capture_attention'] = True
